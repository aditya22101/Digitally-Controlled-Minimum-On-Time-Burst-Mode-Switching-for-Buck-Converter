...
module buck_siso_ctrl_pi_burst #(
    ...
    parameter signed [15:0] Ki_Q8 = 16'd32,
    ...
    reg signed [23:0] integral = 0;
    reg signed [31:0] pi_sum;
    reg signed [15:0] pi_q8, pi_sat;
    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            integral <= 0;
            duty_req <= BASE_DUTY;
        end else if (pwm_tick && pwm_cnt == 0) begin

            error_s <= -$signed({1'b0, adc_in});

            integral <= integral + error_s;
            if (integral > 24'sd50000) integral <= 24'sd50000;
            if (integral < -24'sd50000) integral <= -24'sd50000;

            pi_sum = (Kp_Q8 * error_s) + (Ki_Q8 * integral);
            pi_q8  = pi_sum[23:8];

            if      (pi_q8 > P_MAX) pi_sat <= P_MAX;
            else if (pi_q8 < P_MIN) pi_sat <= P_MIN;
            else                   pi_sat <= pi_q8;

            duty_cand = BASE_DUTY + pi_sat;
